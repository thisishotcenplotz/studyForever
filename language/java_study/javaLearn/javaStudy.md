...
##### Java Reflection
1. 反射机制允许程序在执行期间借助于Reflection API 取得任何类的内部信息（比如成员变量，构造器，成员方法等等），并能操作对象的属性及方法。反射在设计模式和框架底层都会用到
2. 加载完类之后，在堆中就产生了一个Class类型的对象（一个类只有一个Class对象），这个对象包含了类的完整结构信息。通过这个对象得到类的结构。这个对象就像一面镜子，透过这个镜子看到类的结构，所以，形象的被称之为：反射
...
##### 反射的优缺点
1. 优点可以动态的创建和使用对象（也是框架底层的核心），使用灵活，没有反射机制，框架技术就失去底层支撑
...
6. Class对象时存放在堆的
7. 类的字节码二进制数据，是存放在方法区的，有的地方称为类的元数据（包括方法代码、变量名、方法名、访问权限等等）

##### 获取Class类对象

1. 前提：已知一个类的全类名，且该类在类路径下，可以通过Class类的静态方法forName() 获取，可能抛出ClassNotFoundException； ```应用场景：多用于配置文件，读取类全路径，加载类```
2. 前提：若已知具体的类，通过类的class获取，该方式最为安全可靠，程序性能最高： Class cls = Cat.class;  ```应用场景：多用于参数传递，比如通过反射得到对应构造器对象```
3. 前提：已知某个类的实例，调用该实例的getClass() 方法获取Class对象。```应用场景：通过创建好的对象，获取Class对象```
4. 其他方式：
...
##### 数据库三层架构
1. 所谓数据库，就是在主机安装一个数据库管理系统(DBMS)，这个管理程序可以管理多个数据库
...
COLLATE:
    - 'utf8_bin': 区分大小写
    - 'utf8_general_ci': 不区分大小写 （默认值）

##### 数据库备份

```shell
# backup 
mysqldump -u xxx -p xxx -B db1 db2 db3 > xxx.sql
mysqldump -u xxx -p xxx  db1 tb1 tb2 > xxx.sql
...
事务用于保障数据的一致性，它由一组相关的dml语句组成，该组的dml语句要么全部成功，要么全部失败。

- 事务和锁
  - 当执行事务操作时，mysql会在表上加锁，防止其它用户改表数据。这对用户来讲是非常重要的
  - mysql数据库控制台事务的几个重要操作
    ...
    - rollback 全部回退
    - commit  提交事务
      - 使用commit 语句可以提交事务。当执行了commit语句之后，会确认事物的变化、结束任务、删除保存点、释放锁、数据生效。commit结束之后，其他会话可以查看到事务变化后的新数据
      - 也就是说，开启事务后，不commit，其他人看不到变化后的数据，看的都是变化前的。
- 事务隔离级别
  - 多个连接开启各自事务操作数据库中数据时，数据库系统要负责隔离操作，以保证各个链接在获取数据时的准确性
  - 如果不考虑隔离性，可能会引发如下问题
    - 脏读：当一个事物读取另一个事物尚未提交的修改时，产生脏读
    - 不可重复读：同一个查询在同一事物中多次进行，由于其他提交事务所做的修改或删除，每次返回不同结果集，此时发生不可重复读
    - 幻读：同一查询在同一事务中多次进行，由于其他提交事务所做的插入操作，每次返回不同结果集，此时发生幻读。

| 隔离级别                   | 脏读 | 不可重复读 | 幻读 |  锁  |
|:-----------------------|:--:|:-----:|:--:|:---:|
| 读未提交(Read uncommitted) | v  |   v   | v  |  x  |
| 读已提交(Read committed)   | x  |   v   | v  |  x  |
| 可重复读(Repeatable read)  | x  |   x   | x  |  x  |
| 可串行化(Serializable)     | x  |   x   | x  |  v  |


# 第二十五章：JDBC 和 数据库连接池

1. JDBC为访问不同的数据库提供了统一的接口，为使用者屏蔽了细节问题。
2. Java使用JDBC，可以链接任何提供了JDBC驱动程序的数据库系统，从而完成对数据库的各种操作。
...
2. 在连接建立后，需要对数据库进行访问，执行命名或SQL语句，可以通过
    - Statement [存在SQL注入问题]
    - PreparedStatement [预处理]
    - CallableStatement [可以调存储过程]
3. Statement对象执行SQL语句，存在SQL注入风险
4. SQL注入是利用某些系统没有对用户输入的数据进行充分的检查，而在用户输入数据中注入非法的SQL语句或命令，恶意攻击数据库
5. 要防范SQL注入，只要用PreparedStatement（从Statement扩展而来）取代Statement就可以了。

##### 事务
1. JDBC程序当中一个Connection对象创建时，默认情况下是自动提交事务:每次执行一个sql语句时，如果执行成功就会向数据库自动提交，而且不能回滚
2. JDBC程序中为了让多个SQL语句作为一个整体执行，需要使用事务
3. 调用Connection 的setAutoCommit 来控制是否自动提交事务
4. 在所有SQL语句都执行成功后，调用commit 即可
5. 如果失败 则调用rollback

##### 批处理
1. 当需要成批插入或者更新记录时，可以采用Java的批处理更新机制，这一机制允许多条语句一次性执行。通常情况下这种方式比单独处理效率更高。
2. JDBC的批处理语句包括下面方法
    - addBatch(): 添加需要批量处理的SQL
    - executeBatch():执行批量处理语句
    - clearBatch():清空批处理包的语句
3. JDBC 链接MySQL时，如果要使用批处理功能，需要在url中添加``` ?rewriteBatchedStatements=true```
...
传统获取Connection问题分析：
1. 传统的JDBC数据库连接使用DriverManager来获取，每次向数据库建立连接的时候都要将Connection加载到内存中，再验证IP地址、用户名和密码（0.05s~1s）。需要数据库连接的时候，就向数据库要求一个，频繁的进行数据库连接操作，将占用很多的系统资源，容易造成服务器崩溃。
2. 每一次数据库连接，使用完后都得断开，如果程序出现异常而未能关闭，将导致数据库内存泄漏，最终将导致重启数据库。
3. 传统获取连接的方式，不能控制创建的链接数量，如果链接过多，也可能导致内存泄漏，MySQL崩溃
4. 解决传统开发中的数据库连接问题，可以采用数据库连接池技术

数据库连接池基本介绍
1. 预先在缓冲池中放入一定数量的链接，当需要建立数据库连接时，只需要从缓冲池中取一个，使用完再放回去。
2. 数据库连接池负责分配、管理和释放数据库连接，它允许应用程序重复使用一个现有的数据库连接，而不是建立一个新的。
3. 当应用程序向连接池请求的链接数超过最大的连接数量时，这些请求将被加入到等待队列中。
...
##### 介绍
1. 一个正则表达式，就是用某种模式去匹配字符串的一个公式。很多人因为它们看上去比较古怪且复杂所以不敢去使用，不过，经过练习后，就觉得这些复杂的表达式写起来还是相当简单的，而且，一旦你弄懂它们，你就能把数小时的辛苦而且容易错的文本处理工作缩短到几分钟。
...
元字符(Metacharacter) - 转义符 ```\\```
 - 符号说明：在使用正则表达式检索某些特殊字符的时候，需要用转义符号，否则检索不到结果
 - 常见需要用到转义符号的字符有```. * + () $ / \ ? [] ^ {} ``` 等特殊符号

| 符号        |                符号                |         示例         |             解释              |
|:----------|:--------------------------------:|:------------------:|:---------------------------:|
| ```[]```  |             可接收的字符列表             |    ```[efgh]```    |       e,f,g,h中任意一个字符        |
| ```[^]``` |            不可接收的字符列表             |    ```[^abc]```    | 除了a,b,c 之外的任意一个字符，包括数字和特殊符号 |
| ```-```   |               连字符                |     ```A-Z```      |          任意单个大写字母           |
| ```.```   |           匹配除了\n以外任意字符           |     ```a..b```     | 以a开头，b结尾中间包括2个任意字符的长度为4的字符串 |
| ```\\d``` |             匹配单个数字字符             | ```\\d{3}(\\d)?``` |        包含3个或4个数字的字符         |
| ```\\D``` |       匹配单个非数字字符，相当于[^0-9]        |  ```\\D(\\d)*```   |    以单个非数字字符开头，后接任意个数字字符串    |
| ```\\w``` |  匹配单个数字、大小写字母字符，等价于[0-9a-zA-Z]   | ```\\d{3}\\w{4}``` |   以3个数字字符开头的长度为7的数字字母字符串    |
| ```\\W``` | 匹配单个非数字、大小写字母字符，等价于 [^0-9a-zA-Z] |  ```\\W+\\d{2}```  | 以至少1个非数字字母符号开头，2个数字字符结尾的字符串 |
...
| 符号          |       含义       |     示例      |              说明              |         匹配输入         |
|:------------|:--------------:|:-----------:|:----------------------------:|:--------------------:|
| ```*```     |  指定字符重复0次或n次   |   (abc)*    |     仅包含任意个abc的字符，等价于\w*      |    abc,abcabcabc     |
| ```+```     |  指定字符重复1次或n次   |  m+(abc)*   |     以至少1个m开头，后接任意个abc字符串     |    m,mabc,mabcabc    |
| ```?```     | 指定字符重复0次或最多1次  |   m+abc?    |    以至少1个m开头，后接ab或abc的字符串     | mab,mabc,mmmab,mmabc |
...
| 符号        |     含义      |        示例        |                说明                |      匹配输入     |
|:----------|:-----------:|:----------------:|:--------------------------------:|:-------------:|
| ```^```   |   指定起始字符串   |  ^[0-9]+[a-z]*   |       以至少1个数字开头，后面接任意个小写字母       | 123,6aa,555edf |
| ```$```   |   指定结束字符串   | ^[0-9]\\-[a-z]+$ |     以1个数字后接-，并以至少一个小写字母结尾的字符     |      1-a      |
| ```\\b``` | 匹配目标字符串的边界  |      ooo\\b      | 这里说的字符的边界指的是子串间有空格，或者是目标字符串结束的位置 | sdfooo cccooo |
| ```\\B``` | 匹配目标字符串的非边界 |      han\\B      |             和\b相反的含义             |  sdfooo cccooo  |


# 第二十八章：算法优化体验-骑士周游

##### 算法优化的意义
1. 算法是程序员的灵魂，为什么有些程序可以在海量数据计算时，依然保持高速？
2. 算法有很多，比如八大排序（冒泡、选择、插入、快排、并归、希尔、技术、堆排序）、查找算法、分治算法、动态规划、KMP算法、贪心算法、普里姆算法、克鲁斯卡尔算法、迪杰斯特拉算法、佛洛依德算法
3. 骑士周游会使用图的遍历算法（DFS）+ 贪心算法优化

##### 算法介绍
1. 马踏棋盘问题实际上是图的深度优先搜索（DFS）的应用
2. 如果使用回溯（就是深度优先搜索）来解决，假如马儿踏了53个点，发现已经走到尽头，没办法，就需要回退，查看其他路径，就在棋盘上不停回溯
